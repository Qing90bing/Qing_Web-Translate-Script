...
(() => {
  ...
  var aistudioGoogleCom = [
    // 正则表达式翻译
    [/Knowledge cut off: (.+)/i, "知识截止日期：$1"],
    [/^Copied\s+models\/(.+)\s+to\s+clipboard$/i, "已将模型“$1”复制到剪贴板"],
    [/Last modified:\s*(\d{4})\/(\d{1,2})\/(\d{1,2})/, "最后修改于：$1年$2月$3日"],
    ["Google Account: (.+?) \\((.+?)\\)", "Google 帐号：(.+?) \\((.+?)\\)"],
    ...
    ["The configuration is for working with React + TypeScript application. Assumes a basic structure with index.html and index.tsx. Code Assistant follows strict guidelines for using the Gemini API", "该配置用于使用React + TypeScript应用程序,假设具有index.html和index.tsx的基本结构,代码助理遵循严格使用Gemini API的准则"],
    ...
    ["Individual customer availability may vary depending on billing status and surface used: free tier, billed tier, as well as the chosen model and API features in use.", "可用性可能因您的计费状态、服务级别（例如免费套餐、付费套餐）以及所选用的模型和 API 功能而异。"],
    ...
    ["This feature enables the model to choose to not respond to audio that’s not relevant to the ongoing conversation", "此功能可让模型选择性地忽略与 当前对话无关的音频。"],
    ...
    ["Enable function calling to get automatically generated responses for your function calls.", "启用函数调用功能，可为您的函数调用自动生成响应。"],
    ["Higher resolutions may provide better understanding but use more tokens.", "更高的分辨率有助于模型更好地理解内容，但会消耗更多 Token (令牌)。"],
    ...
    ["Let the model decide how many thinking tokens to use or choose your own value", "让模型自行决定使用多少“思考 token”，或由您手动设定。"],
    ...
    ["Let Gemini adapt its response style to the input expression and tone", "让 Gemini 根据输入内容的表达和语气调整回应风格。"],
    ...
    ["Lets Gemini use code to solve complex tasks", "允许 Gemini 调用代码执行器来解决复杂任务"],
    ...
    ["Truncate response including and after string", "从指定字符串处截断回复（包含该字符串）"],
    ...
    ["Get SDK code to interact with Gemini Live", "获取与 Gemini Live 交互的 SDK 代码"],
    ...
    ["Probability threshold for top-p sampling", "Top-p 核采样的概率阈值"],
  ...
  var consoleAnthropicCom = [
    ...
    ["Sign in or create a developer account to build with the Anthropic API", "登录或创建开发者账号以使用 Anthropic API 构建"],
    ...
    ["Build with Claude", "使用 Claude 构建"],
    ["Build with the Anthropic API, an AI assistant from Anthropic", "使用 Anthropic API 构建，它是一款来自 Anthropic 的 AI 助手"],
    ...
    ["Your privacy choices", "您的隐私选择"],
    ["Anthropic Console", "Anthropic 控制台"],
    ["Build with the Anthropic API, an AI assistant from Anthropic", "使用 Anthropic API 构建，一款来自 Anthropic 的 AI 助手"],
