...
const EMBEDDED_TRANSLATIONS = {
  'zh-cn': {
    'aistudio.google.com': {
      ...
      textRules: [
        ['Image output is priced at $60 per 1,000,000 Tokens. Output images at 0.5K (512x512px) consume 747 tokens and are equivalent to $0.045 per image. Output images at 1K (1024x1024px) consume 1120 tokens and are equivalent to $0.067 per image. Output images at 2K (2048x2048px) consume 1680 tokens and are equivalent to $0.101 per image. Output images at 4K (4096x4096px) consume 2520 tokens and are equivalent to $0.151 per image.', '图像输出定价为每 1,000,000 个令牌 60 美元。0.5K（512x512 像素）的输出图像消耗 747 个令牌，相当于每张图像 0.045 美元。1K（1024x1024 像素）的输出图像消耗 1120 个令牌，相当于每张图像 0.067 美元。2K（2048x2048 像素）的输出图像消耗 1680 个令牌，相当于每张图像 0.101 美元。4K（4096x4096 像素）的输出图像消耗 2520 个令牌，相当于每张图像 0.151 美元。'],
        ...
        ['Powered by the Lyria AI model, this Stepmania-inspired experience dynamically generates high-fidelity music tracks and custom beatmaps on the fly. Master an infinite library of unique songs, tailoring the tempo, genre, and mood to push your reflexes to the absolute limit.', '由 Lyria AI 模型驱动，这个受 Stepmania 启发的体验动态生成高保真音乐曲目和自定义节拍图。掌握无限独特歌曲库，调整节奏、流派和情绪，将您的反应能力推向极限。'],
        ...
        ['Gemini API is having some issues serving recently created keys, including when used via OpenAI libraries. Investigation is underway. You can create a new key in AI Studio if your recently created key is not working.', 'Gemini API 在服务最近创建的密钥时遇到一些问题，包括通过 OpenAI 库使用时。调查正在进行中。如果最近创建的密钥无法使用，您可以在 AI Studio 中创建新密钥。'],
        ...
        ['Several issues have been identified affecting the Build experience. Users may notice slower agent responses when using an API key, occasional response clipping, and intermittent errors. Multiple mitigations are being deployed.', '已识别出影响 Build 体验的多个问题。用户在使用 API 密钥时可能会注意到代理响应变慢、偶尔响应截断和间歇性错误。正在部署多项缓解措施。'],
        ['Fill cells to reveal a hidden image. Numbers show the size and order of solid blocks (e.g., 3 1 = a block of 3, then 1). Blocks must have at least one gap between them. Use X to mark guaranteed empty spaces.', '填充单元格以揭示隐藏图像。数字显示实心块的大小和顺序（例如，3 1 = 一个 3 块，然后 1）。块之间必须至少有一个间隙。使用 X 标记保证为空的空间。'],
        ...
        ['Build with our new, SOTA deep research agents which offer significantly higher report quality, visual outputs, and deeper integrations with your existing data and tools, including MCPs support.', '使用我们新的、最先进的深度研究代理进行构建，这些代理提供显著更高的报告质量、视觉输出，以及与现有数据和工具（包括 MCP 支持）的更深入集成。'],
        ...
        ['API pricing per 1M tokens. Audio tokens correspond to 25 Tokens per second of audio. Equivalent to $0.0368 per minute.\nUsage in AI Studio UI is free of charge when no API key is selected', '每 100 万个令牌的 API 定价。音频令牌相当于每秒音频 25 个令牌。相当于每分钟 0.0368 美元。\n未选择 API 密钥时，AI Studio 界面中的使用是免费的'],
        ...
        ['An elegant music generation playground app that transforms prompts into complete songs, featuring an intuitive guided mode, high-quality video exports, and karaoke-style outputs for any track."', '一个优雅的音乐生成游乐场应用，将提示转换为完整的歌曲，具有直观的引导模式、高质量的视频导出和卡拉OK风格的输出。'],
        ...
        ['Autonomous financial research Antigravity agent showing strong multimodality skills that gathers SEC filings, processes reasoning timelines, and generates interactive stock analysis reports.', '自主金融研究 Antigravity 代理，展示强大的多模态技能，收集 SEC 文件，处理推理时间线，并生成交互式股票分析报告。'],
        ['Create next-generation video with Gemini Omni Flash. Bring text and images to life as stunning video, with conversational editing for complete, easy-to-steer creative control.', '使用 Gemini Omni Flash 创建下一代视频。将文本和图像生动地呈现为令人惊叹的视频，并通过对话式编辑实现完全、易于掌控的创意控制。'],
        ...
        ['Introducing Nano Banana 2 Lite for ultra-fast, cost-efficient image generation, and Gemini Omni Flash for powerful video creation with conversational, step-by-step editing', '推出 Nano Banana 2 Lite，用于超快速、经济高效的图像生成，以及 Gemini Omni Flash，用于通过对话式、逐步编辑进行强大的视频创作'],
        ['Introducing our latest generative media models: scale high-velocity image creation with Nano Banana 2 Lite, and create, tweak, or perfect your videos using Gemini Omni Flash', '推出我们最新的生成式媒体模型：使用 Nano Banana 2 Lite 扩展高速图像创作，并使用 Gemini Omni Flash 创建、调整或完善您的视频'],
        ...
        ['Use your pen to control a virtual OS. Create AI wallpapers, take actions on folders, and summarize emails, leveraging the multimodal understanding capabilities of Gemini 3 Flash.', '使用您的笔控制虚拟操作系统。创建 AI 壁纸、对文件夹执行操作并总结电子邮件，利用 Gemini 3 Flash 的多模态理解能力。'],
        ['We are experiencing issues with apps in AI Studio Build. Users may see errors or timeouts when attempting to create, open, run, or deploy applications. Investigation is underway.', 'AI Studio Build 中的应用遇到问题。用户在尝试创建、打开、运行或部署应用时可能会看到错误或超时。调查正在进行中。'],
        ...
        ['An interactive reading experience where any text highlight instantly generates an AI-powered explanation and a contextual image representation in a seamless floating tooltip.', '一种交互式阅读体验，任何文本高亮都会立即生成 AI 驱动的解释和上下文图像表示，并显示在无缝浮动的工具提示中。'],
        ...
        ['We are experiencing issues with Nano Banana 2 and Nano Banana Pro models on Gemini API and AI Studio when using 2k or 4k resolution. Investigation is underway.', '在使用 2k 或 4k 分辨率时，Gemini API 和 AI Studio 上的 Nano Banana 2 和 Nano Banana Pro 模型遇到问题。调查正在进行中。'],
        ...
        ['We are experiencing increased latency for Gemini API requests. Additionally, users on API keys may be charged for more tokens than usual due to decreased cache efficiency.', 'Gemini API 请求的延迟增加。此外，由于缓存效率降低，使用 API 密钥的用户可能会被收取比平时更多的令牌费用。'],
        ...
        ['A top-down mini-golf game where levels are shaped like characters. Features physics-based gameplay, drag-to-shoot controls, and an AI caddy powered by Gemini.', '一款俯视视角的迷你高尔夫游戏，关卡形状像角色。具有基于物理的游戏玩法、拖拽射击控制，以及由 Gemini 驱动的 AI 球童。'],
        ...
        ['An interactive 3D spherical gallery of images using WebGL. Features free drag-to-rotate interaction, momentum scrolling, and deep zoom camera penetration inside the sphere.', '使用 WebGL 的交互式 3D 球形图像画廊。具有自由拖拽旋转交互、动量滚动和球体内深度缩放相机穿透功能。'],
        ...
        ['Brush, feed, play and care for your Dino to earn in game currency. Redeem it in the shop and raise your Dino to the highest level you can before it grows up.', '刷洗、喂食、玩耍并照顾你的恐龙以赚取游戏货币。在商店中兑换它，并在它长大之前将你的恐龙提升到尽可能高的等级。'],
        ...
        ['Generate complete radio shows from simple prompts, featuring dynamic background music, realistic speech, and rich visuals using the new Antigravity agent.', '使用新的 Antigravity 代理，从简单的提示生成完整的广播节目，包括动态背景音乐、逼真的语音和丰富的视觉效果。'],
        ...
        ['A smart audio transcription tool using Gemini 3 Flash to provide accurate speech-to-text with speaker diarization, timestamps, and language detection.', '一款使用 Gemini 3 Flash 的智能音频转录工具，提供准确的语音转文本，支持说话人分离、时间戳和语言检测。'],
        ...
        ['A clean, minimalist weather app with a hidden Multiverse prompt bar that uses Gemini API to completely overhaul its aesthetic and tone in real-time.', '一款简洁、极简的天气应用，带有隐藏的 Multiverse 提示栏，使用 Gemini API 实时彻底改变其美学和基调。'],
        ...
        ['Stretch the cat and fill every space on the board, then end at the finish flag. Gather fish and stars to earn extra time as you race against the clock.', '拉伸猫咪，填满棋盘上的每个空间，然后在终点旗处结束。收集鱼和星星以在与时间赛跑时赢得额外时间。'],
        ['Connect letters in a continuous line to form words. Guess the right words and they’ll fill in the crossword grid. Solve each word to complete the level!', '用连续的线连接字母组成单词。猜对单词，它们就会填满填字游戏网格。解决每个单词即可完成关卡！'],
        ['The oldest logs are automatically deleted on a rolling basis according to the selected retention timeframe below, or sooner if the storage limit is reached.', '最旧的日志将根据下方选定的保留时间范围自动滚动删除，如果达到存储限制，则可能提前删除。'],
        ['AI Data Analyst delivers interactive data analytics and insights. Upload CSVs, ask a question, and let Gemini Managed Agents autonomously analyze it.', 'AI 数据分析师提供交互式数据分析和洞察。上传 CSV 文件，提出问题，让 Gemini 托管代理自主分析。'],
        ...
        ['Generate fast, natural speech with Gemini 3.1 Flash TTS. Apply new expressive audio tags for complete, easy-to-steer narration control.', '使用 Gemini 3.1 Flash TTS 生成快速、自然的语音。应用新的表现力音频标签，实现完全、易于掌控的叙述控制。'],
        ...
        ['Burn calories, not just time! Experience the Chrome-Dino-like game powered by MediaPipe - jump in real life to dodge cacti!', '燃烧卡路里，而不仅仅是时间！体验由 MediaPipe 驱动的类似 Chrome 恐龙的游戏——在现实生活中跳跃以躲避仙人掌！'],
        ...
        ['InTute is your steerable, conversational, visual tutor, who can teach any subject in any language. Just speak and ask for what you want.', 'InTute 是您的可操控、对话式、视觉导师，可以用任何语言教授任何科目。只需说出您想要的内容即可。'],
        ['Talk to a smart, collaborative agent who will write for you. Communicate thoughts, brainstorm ideas and create interactive multimodal documents.', '与一个智能、协作的代理交谈，它将为您写作。交流想法、集思广益并创建交互式多模态文档。'],
        ...
        ['Byte Beats is an interactive music creation app that lets you generate 8-bit or 16-bit retro arcade beats and jam on a 8-pad grid.', '字节节拍是一款交互式音乐创作应用，可让您生成 8 位或 16 位复古街机节拍，并在 8 格网格上即兴演奏。'],
        ...
        ['Redesign your room in clicks using Gemini image generation. Swipe through styled interior design variations of your own space.', '使用 Gemini 图像生成，只需点击几下即可重新设计您的房间。滑动浏览您自己空间的各种风格室内设计变体。'],
        ...
        ['An interactive image editing application using Gemini Live to interpret voice and pointing gestures for real-time image manipulation.', '一个交互式图像编辑应用程序，使用 Gemini Live 来解释语音和指向手势，实现实时图像操作。'],
        ['Find the perfect voice for your Text to Speech needs. Simply describe the voice you are looking for and the AI will do the casting for you.', '为您的文本转语音需求找到完美的声音。只需描述您要找的声音，AI 就会为您完成选角。'],
        ['Our latest SOTA reasoning model with unprecedented depth and nuance, and powerful multimodal understanding and coding capabilities', '我们最新的 SOTA 推理模型，具有前所未有的深度和细微差别，以及强大的多模态理解和编码能力'],
        ...
        ['Our latest model that balances speed with intelligence to deliver strong performance in agentic and multimodal tasks is now available.', '我们最新的模型在速度与智能之间取得平衡，在代理和多模态任务中表现出色，现已推出。'],
        ...
        ['Play hide and seek against Gemini on Google Maps. Use streetview images and directional clues to guess where Gemini is hiding.', '在 Google 地图上与 Gemini 玩捉迷藏。使用街景图像和方向线索来猜测 Gemini 藏在哪里。'],
        ...
        ['With this disabled, GenerateContent API (standard Gemini API) requests are not stored. To override this, set', '禁用此选项后，GenerateContent API（标准 Gemini API）请求将不会被存储。要覆盖此设置，请在 API 中设置'],
        ...
        ['An interactive application using Gemini Live to interpret voice and pointing gestures to search for places visible within images', '一个交互式应用，使用 Gemini Live 解释语音和指向手势，以搜索图像中可见的地点。'],
        ...
        ['We are experiencing issues with Gemini API context caching resulting in higher token usage than expected. Mitigation is underway.', 'Gemini API 上下文缓存遇到问题，导致令牌使用量高于预期。缓解措施正在进行中。'],
        ...
        ['Stream live radio broadcasts from anywhere in the world and translate them into your chosen language in real time using Gemini AI.', '使用 Gemini AI 实时流式传输来自世界各地的广播，并将其翻译成您选择的语言。'],
        ['Tessera is an interactive art workstation that generates unique artistic mosaic tiles & lets you create a sharable poster', '镶嵌画是一个交互式艺术工作站，可生成独特的艺术马赛克瓷砖，并让您创建可分享的海报。'],
        ['A real-time hair color changer using MediaPipe Hair Segmentation. Try on different hair colors instantly using your webcam.', '使用 MediaPipe 头发分割的实时发色更换器。使用您的网络摄像头即时尝试不同的发色。'],
        ['A beautiful personal blog template with immersive article views, galleries, and a mix of serif and sans-serif typography.', '一个漂亮的个人博客模板，具有沉浸式文章视图、画廊以及衬线和无衬线字体的混合排版。'],
        ...
        ['Generate AI-powered images and videos from text prompts. Customize this template to build your own media generation app.', '根据文本提示生成 AI 驱动的图像和视频。自定义此模板以构建您自己的媒体生成应用。'],
        ...
        ['Experience our fastest image generation model yet, engineered to deliver high-quality visuals at rapid speed and scale.', '体验我们迄今为止最快的图像生成模型，以快速和规模提供高质量视觉效果。'],
        ...
        ['An infinite spatial canvas for exploring interconnected concepts using AI-generated text and contextual imagery.', '一个无限的空间画布，用于使用 AI 生成的文本和上下文图像探索相互关联的概念。'],
        ...
        ['Turn product photos into cinematic vibe reels with Gemini Omni Flash and Nano Banana Flash Lite.', '使用 Gemini Omni Flash 和 Nano Banana Flash Lite 将产品照片转换为电影般的氛围短片。'],
        ...
        ['A SaaS agent that plans and executes multi-step tasks, adaptable to a wide range of real-world workflows.', '一个 SaaS 代理，可规划和执行多步骤任务，适用于各种现实世界的工作流程。'],
        ...
        ['API pricing per 1M tokens.\nUsage in AI Studio UI is free of charge when no API key is selected', '每 100 万个令牌的 API 定价。\n未选择 API 密钥时，AI Studio 界面中的使用是免费的'],
        ...
        ['Try our SOTA general-purpose autonomous agent running in a remote, Google-hosted Linux environment.', '尝试我们最先进的通用自主代理，它在远程、Google 托管的 Linux 环境中运行。'],
        ...
        ['Build live multilingual experiences where global audiences stream audio in their own language instantly.', '构建实时多语言体验，让全球受众即时以他们自己的语言流式传输音频。'],
        ...
        ['Transforms a text source into a polished, simulated radio show with hosts, callers, and background music.', '将文本源转换为带有主持人、来电者和背景音乐的精致模拟广播节目。'],
        ...
        ['A 3D track racing game illustrating use of Full-Stack Runtime to support realtime multiplayer gameplay.', '一款 3D 赛道赛车游戏，展示了使用全栈运行时支持实时多人游戏。'],
        ...
        ['Organize and import recipes with ease. Collaborate with your household and keep your culinary traditions alive.', '轻松整理和导入食谱。与家人协作，保持您的烹饪传统。'],
        ...
        ['A high-octane voice-controlled infinite runner where your volume controls your jetpack thrust.', '一款高强度的语音控制无尽跑酷游戏，您的音量控制喷气背包的推力。'],
        ['Explore top global cities and save destinations using this Google Maps Platform travel planner', '使用此 Google 地图平台旅行规划器探索全球顶级城市并保存目的地。'],
        ['Prompt AI Studio to integrate MediaPipe cross-platform optimized ML solutions automatically.', '提示 AI Studio 自动集成 MediaPipe 跨平台优化的机器学习解决方案。'],
        ...
        ['Our fastest, most cost-effective 3.5 model for high-throughput execution is now available.', '我们最快、最具成本效益的 3.5 模型，适用于高吞吐量执行，现已推出。'],
        ['Search for matches across text, images, and audio files using the Gemini Embedding 2 model.', '使用 Gemini Embedding 2 模型在文本、图像和音频文件中搜索匹配项。'],
        ...
        ['Scans a website to build a custom knowledge base and answer support questions using that content.', '扫描网站以构建自定义知识库，并使用该内容回答支持问题。'],
        ['Users are experiencing increased error rates for DeepResearch requests on Interactions API.', '用户在 Interactions API 上遇到 DeepResearch 请求错误率增加。'],
        ['Issue has been detected involving increased quota errors for multiple models. Investigation is underway.', '检测到多个模型的配额错误增加。调查正在进行中。'],
        ...
        ['Analyzes your codebase to identify issues, answer questions, and generate bug-fixing patches.', '分析你的代码库以识别问题、回答问题并生成修复错误的补丁。'],
        ...
        ['A general-purpose autonomous agent running in a remote, Google-hosted Linux environment.', '一个在远程、Google 托管的 Linux 环境中运行的通用自主代理。'],
        ['Issues have been detected. Users may experience hanging requests. Investigation is underway.', '已检测到问题。用户可能会遇到请求挂起。调查正在进行中。'],
        ['Some users may see temporary errors when viewing Spend Cap. Investigation is underway.', '部分用户在查看支出上限时可能会看到临时错误。调查正在进行中。'],
        ...
        ['Developer tool for scaling localized marketing assets using Gemini 3.1 Flash Image.', '使用 Gemini 3.1 Flash Image 扩展本地化营销资产的开发人员工具。'],
        ['Infrastructure outage affecting AI Studio and Gemini API. Investigation is underway.', '影响 AI Studio 和 Gemini API 的基础设施中断。调查正在进行中。'],
        ...
        ['Grab the most valuable prizes, and climb the leaderboard in this 3D claw machine game.', '在这个 3D 抓娃娃机游戏中抓取最有价值的奖品，并登上排行榜。'],
        ...
        ["An augmented reality experience using MediaPipe to juggle '6' and '7' with your hands.", '使用 MediaPipe 的增强现实体验，用双手抛接“6”和“7”。'],
        ...
        ['API request storage enables observability (logs) and server-side state management.', 'API 请求存储可实现可观测性（日志）和服务器端状态管理。'],
        ['Ask Gemini to find any place on Earth and explore it instantly on Google Maps.', '让 Gemini 寻找地球上的任何地方，并即时在 Google 地图上探索'],
        ['Build your own AI app in minutes, then share it with a friend or with the world.', '在几分钟内构建你自己的 AI 应用，然后与朋友或全世界分享。'],
        ...
        ['Builds live weather dashboards on demand with real-time forecasts and historical data.', '按需构建实时天气仪表板，包含实时预报和历史数据。'],
        ...
        ['Number of impacted requests has reduced from 20% to 10%. Mitigiations are underway.', '受影响请求数已从 20% 降至 10%。缓解措施正在进行中。'],
        ...
        ['Real-time facial analysis and emotion tracking using MediaPipe and Gemini AI.', '使用 MediaPipe 和 Gemini AI 进行实时面部分析和情绪跟踪。'],
        ['Simulated Franka Panda executing Pick and Place using Gemini Robotics ER.', '模拟的 Franka Panda 使用 Gemini Robotics ER 执行拾取与放置。'],
        ...
        ['Gemini API is experiencing issues on batch endpoints. Investigation is underway.', 'Gemini API 在批处理端点上遇到问题。调查正在进行中。'],
        ...
        ['AI Studio Build outage. Apps may not load. Mitigations are underway.', 'AI Studio Build 中断。应用可能无法加载。缓解措施正在进行中。'],
        ['Apps or services using this key will stop working. This action cannot be undone.', '使用此密钥的应用或服务将停止工作，此操作无法撤销'],
        ['Uses computer vision to recognize objects and makes music based on what it sees.', '使用计算机视觉识别物体，并根据所见内容创作音乐。'],
        ['With this disabled, Interactions API requests are not stored, disabling', '禁用此选项后，Interactions API 请求将不会被存储，从而禁用'],
        ...
        ['Collect orbs to dominate the neon grid in this real-time multiplayer snake game.', '在这个实时多人蛇游戏中收集球体以主宰霓虹网格。'],
        ...
        ['Increased error rates and latency for Gemini API. Mitigations are underway.', 'Gemini API 错误率和延迟增加。缓解措施正在进行中。'],
        ['Multiple AI Studio features are experiencing issues. Investigation is underway.', '多个 AI Studio 功能遇到问题。调查正在进行中。'],
        ['Storage is NOT enabled for either the GenerateContent or Interactions API.', 'GenerateContent 或 Interactions API 均未启用存储。'],
        ...
        ['A raw WebGL masterpiece featuring a Mandelbulb fractal with overlay fog.', '一个原始 WebGL 杰作，具有曼德尔球分形和叠加雾效果。'],
        ...
        ['Send your pet on a global adventure using Gemini 3.1 Flash Image.', '使用 Gemini 3.1 Flash Image 让您的宠物踏上全球冒险之旅。'],
        ...
        ['Generate photorealistic window views based on live weather and specific locations.', '根据实时天气和特定位置生成逼真的窗景。'],
        ...
        ['Imagine any website, generated in real-time by Gemini 3.1 Flash-Lite', '想象任何网站，由 Gemini 3.1 Flash-Lite 实时生成。'],
        [' Gemini 2.5 Flash Image, state of the art image generation and editing ', 'Gemini 2.5 Flash 图像，最先进的图像生成和编辑'],
        ['Explore the thinking-with-images feature of Gemini 3.0 Flash Preview', '探索 Gemini 3.0 Flash Preview 的“图像思考”功能'],
        ...
        ['Increased error rates for DeepResearch requests on Interactions API', 'Interactions API 上 DeepResearch 请求错误率增加'],
        ...
        ['Bring ideas to life quickly with Gemini 3.1 Flash Lite Image', '使用 Gemini 3.1 Flash Lite Image 快速将创意变为现实'],
        ['Generate a scavenger hunt for street food around the city of Seoul, Korea', '为韩国首尔市的街头小吃设计一个寻宝游戏'],
        ['Generate e-commerce wireframes across multiple categories in seconds.', '在几秒钟内生成跨多个类别的电子商务线框图。'],
        ...
        ['Video output price is equivalent to $0.10 per second of video (720p)', '视频输出价格相当于每秒视频（720p）0.10 美元'],
        ...
        ['Moves to Google Drive Trash (Permanently deleted after 30 days)', '移至 Google 云端硬盘回收站（30 天后永久删除）'],
        ...
        ['Gemini API is having some issues serving recently created keys', 'Gemini API 在服务最近创建的密钥时遇到一些问题'],
        ...
        ['Send prompt (Ctrl + Enter)\nAppend to prompt (Alt + Enter)', '发送提示 (Ctrl + Enter)\n附加到提示 (Alt + Enter)'],
        ...
        ['Generate cinematic 3D animations from simple text with Veo 3.1', '使用 Veo 3.1 从简单文本生成电影级 3D 动画'],
        ...
        ['Upload a video and generate a custom musical bed using Lyria.', '上传视频并使用 Lyria 生成自定义音乐背景。'],
        ['Users are experiencing unavailability issues with Gemini 2.5 Pro', '用户正遇到 Gemini 2.5 Pro 不可用的问题'],
        ...
        ['AI Studio Build outage. 10% of agent requests are hanging.', 'AI Studio Build 中断。10% 的代理请求挂起。'],
        ['Extended reasoning for complex queries with Gemini 3.1 Pro', '使用 Gemini 3.1 Pro 对复杂查询进行扩展推理'],
        ...
        ['Lightning-fast responses with Gemini 3.1 Flash-Lite', '使用 Gemini 3.1 Flash-Lite 实现闪电般的快速响应'],
        ['Scale high-velocity image creation with Nano Banana 2 Lite', '使用 Nano Banana 2 Lite 扩展高速图像创作'],
        ...
        ['Issues with streaming Deep Research. Investigation is underway.', '流式深度研究问题。调查正在进行中。'],
        ['Nano Banana 2 Lite and Gemini Omni Flash are here', 'Nano Banana 2 Lite 和 Gemini Omni Flash 现已推出'],
        ['Summarize & extract info from video with Gemini 3.1 Pro', '使用 Gemini 3.1 Pro 从视频中总结和提取信息'],
        ...
        ['Gemini 3.5 Live Translate Preview is now available', 'Gemini 3.5 Live Translate 预览版现已推出'],
        ...
        ['/shortcuts. View keyboard shortcuts. Opens sub-menu', '/shortcuts。查看键盘快捷键。打开子菜单'],
        ['Build chatbots, agents, and code with Gemini 3.', '使用 Gemini 3 构建聊天机器人、代理和代码。'],
        ['Customer upgrade to postpay experiencing service disruption', '客户升级到后付费时遇到服务中断'],
        ...
        ['Real-time audio transcription with Gemini 3.5 Flash', '使用 Gemini 3.5 Flash 进行实时音频转录'],
        ['Start typing a prompt to see what our models can do', '开始输入提示词，看看我们的模型能做什么'],
        ...
        ['AI Studio Spend Cap experiencing intermittent issues.', 'AI Studio 支出上限遇到间歇性问题。'],
        ['Check that the URL was entered correctly and try again', '请检查 URL 是否输入正确，然后重试'],
        ['Discover & remix experiences made with AI Studio', '发现并重新混合使用 AI Studio 制作的体验'],
        ['Explore the Veo 3 gallery and remix an example', '浏览 Veo 3 案例库，并选择一个进行二次创作'],
        ['Fast image generation & editing with Nano Banana 2', '使用 Nano Banana 2 快速生成和编辑图像'],
        ...
        ['Issue has been detected affecting the Build experience.', '检测到影响 Build 体验的问题。'],
        ...
        ['Recent activity. Veo Studio. Vibe coding session', '最近活动。Veo Studio。Vibe 编码会话'],
        ...
        ['/chat. Start a chat with a model. Opens sub-menu', '/chat。与模型开始聊天。打开子菜单'],
        ['A fix was implemented to address the configuration issue.', '已实施修复以解决配置问题'],
        ['AI Studio Build experiencing intermittent issues.', 'AI Studio Build 遇到间歇性问题。'],
        ...
        ['Upload & understand images with Gemini 3.1 Pro', '使用 Gemini 3.1 Pro 上传和理解图像'],
        ...
        ['Meet MediaPipe: on-device ML solutions!', '认识 MediaPipe：设备端机器学习解决方案！'],
        ['Nano Banana 2 Lite and Gemini Omni Flash', 'Nano Banana 2 Lite 和 Gemini Omni Flash'],
        ['Partial outage impacting some AI Studio features', '部分中断影响某些 AI Studio 功能'],
        ['Start using Live API in Google AI Studio', '开始在 Google AI Studio 中使用 Live API'],
        ['Describe an app and let Gemini do the rest', '描述一个应用，让 Gemini 完成其余工作'],
        ['Increased error rates and latency for Gemini API.', 'Gemini API 错误率和延迟增加。'],
        ...
        ['Turn images into cinematic video with Veo 3', '使用 Veo 3 将图像转换为电影级视频'],
        ...
        ['AI Studio Build experiencing latency issues.', 'AI Studio Build 遇到延迟问题。'],
        ['Antigravity Agent Preview is now available', 'Antigravity Agent 预览版现已推出'],
        ...
        ['Real-time voice and video with Live API.', '使用 Live API 实现实时语音和视频。'],
        ...
        ['/build. Vibe code an app. Opens sub-menu', '/build。Vibe 编码应用。打开子菜单'],
        ...
        ['Real-time voice with the Gemini Live API', '使用 Gemini Live API 实现实时语音'],
        ...
        ['/key. Create an API key. Opens sub-menu', '/key。创建 API 密钥。打开子菜单'],
        ['Generate logo and swag ideas for a brand', '为品牌构思 Logo 和周边产品创意'],
        ['Issues with apps in AI Studio Build.', 'AI Studio Build 中的应用出现问题。'],
        ...
        ['The best Gemini model is chosen automatically', '自动选择最佳 Gemini 模型'],
        ['Add Line Comment, Control+K Control+C', '添加行注释, Control+K Control+C'],
        ['AI Studio may not load for some users.', '部分用户可能无法加载 AI Studio'],
        ['CreateDataset failures in AI Studio', 'AI Studio 中的 CreateDataset 失败'],
        ...
        ['Access real-time info with Search Grounding', '使用搜索接地访问实时信息'],
        ['Add Cursor Above, Control+Alt+UpArrow', '在上方添加光标, Control+Alt+↑'],
        ['AI Studio experiencing increased error rates.', 'AI Studio 错误率增加。'],
        ...
        ['Natural-sounding speech with Gemini TTS', '使用 Gemini TTS 实现自然语音'],
        ...
        ['Create video with Gemini Omni Flash', '使用 Gemini Omni Flash 创建视频'],
        ...
        ['New Build experience & Gemini 3.1 Pro', '新的构建体验和 Gemini 3.1 Pro'],
        ...
        ['Access real-time info with Maps Grounding', '使用地图接地访问实时信息'],
        ...
        ['Firestore & Auth with Firebase', '使用 Firebase 的 Firestore 和 Auth'],
        ['Gemini API and AI Studio outage.', 'Gemini API 和 AI Studio 服务中断'],
        ['Gemini API Batch experiencing issues.', 'Gemini API 批处理遇到问题。'],
        ...
        ['Text-to-video generation with Veo 3', '使用 Veo 3 进行文本到视频生成'],
        ...
        ['Build custom Google Docs workflows', '构建自定义 Google Docs 工作流'],
        ['Discover & remix games, websites, and', '发现并重新混合游戏、网站和'],
        ['Example Angular app using Gemini', '使用 Gemini 的 Angular 示例应用'],
        ['Gemini 2.5 Pro Unavailability Incident', 'Gemini 2.5 Pro 不可用事件'],
        ['Gemini API Batch experiencing an outage.', 'Gemini API 批处理中断。'],
        ['Get SDK code to chat with Gemini', '获取 Gemini 聊天功能的 SDK 代码'],
        ['Increased error rates for Build requests.', 'Build 请求错误率增加。'],
        ['Issues with Gemini API context caching', 'Gemini API 上下文缓存问题'],
        ...
        ['Import a GitHub repository as an app', '将 GitHub 仓库导入为应用'],
        ['Start a chat or vibe code an app', '开始聊天或进行 vibe 编码应用'],
        ...
        ['Gemini 3.6 Flash is now available', 'Gemini 3.6 Flash 现已推出'],
        ['in the API to opt-in per request.', '在 API 中按请求选择加入。'],
        ['Increased error rates for model quota.', '模型配额错误率增加。'],
        ...
        ['Access Drive files and folders', '访问 Drive 文件和文件夹'],
        ...
        ['Choose exact image aspect ratios', '选择精确的图像宽高比'],
        ['Deploy app on Google Cloud', '在 Google Cloud 上部署应用'],
        ['Design with Nano Banana 2', '使用 Nano Banana 2 进行设计'],
        ...
        ['Score your video with Lyria', '使用 Lyria 为您的视频配乐'],
        ...
        ['Issues with streaming Deep Research', '流式深度研究问题'],
        ...
        ["Gemini: Google's AI Model", 'Gemini：Google 的 AI 模型'],
        ['Grounding with Google Maps', '使用 Google 地图进行接地'],
        ...
        ['Issues with Gemini API batch', 'Gemini API 批处理问题'],
        ['Model used to generate response', '用于生成回复的模型'],
        ['Nano Banana 2 Lite is here', 'Nano Banana 2 Lite 来了'],
        ['No recording devices available.', '没有可用的录制设备'],
        ['Peak requests per day (RPD)', '每天最大请求次数 (RPD)'],
        ["You don't have any prompts yet.", '您还没有任何提示。'],
        ...
        ['Peak input tokens per minute', '每分钟峰值输入令牌数'],
        ['Read and summarize Chat spaces', '阅读和总结聊天空间'],
        ...
        ['Create tracks with Lyria 3', '使用 Lyria 3 创建曲目'],
        ...
        ['Integrate your slide decks', '集成您的幻灯片'],
        ['Peak requests per minute', '每分钟峰值请求数'],
        ...
        ['Convert text to speech', '将文本转换为语音'],
        ['Create new chat prompt', '创建新的聊天提示'],
        ...
        ['Gemini Agentic Vision', 'Gemini 智能体视觉'],
        ...
        ['Items per page selection', '每页项目数选择'],
        ['Live audio-to-audio dialog', '实时音频对话'],
        ['made with AI Studio', '使用 AI Studio 制作'],
        ['Organize ideas and notes', '组织想法和笔记'],
        ['policy-controlled features', '策略控制功能'],
        ['Prompt name text field', '提示名称文本字段'],
        ['See original conversations', '查看原始对话'],
        ['Send a file to the model', '向模型上传文件'],
        ['The issue has been resolved.', '问题已解决'],
        ['Toggle view all models', '切换查看所有模型'],
        ['Turbo Race Multiplayer', '涡轮竞速多人游戏'],
        ['View and manage settings', '查看并管理设置'],
        ...
        ['World Radio Translator', '世界电台翻译器'],
        ...
        ['Flash-Lite Browser', 'Flash-Lite 浏览器'],
        ...
        ['Infrastructure outage', '基础设施中断'],
        ['Maps Travel Planner', '地图旅行规划器'],
        ['Multimodal Live API', '多模态实时 API'],
        ['Omni Product Studio', '全能产品工作室'],
        ...
        ['AI-Pointer: Find', 'AI 指针：查找'],
        ['Applied filters', '已应用的筛选器'],
