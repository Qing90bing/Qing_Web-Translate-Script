...
async function findTranslationFiles() {
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      console.warn(color.yellow(t('validation.dirNotFound')));
      return [];
    }
    // 对于其他类型的错误，则重新抛出。
    throw error;
...
function validateFileContent(file, content, options) {
  ...
  try {
    ...
    // 捕获语法错误
    errors.push({ file, line: e.loc.line, column: e.loc.column, pos: e.pos, lineContent: content.split('\n')[e.loc.line - 1] || '', message: t('validation.fileUnparseable', e.message.replace(/ \(\d+:\d+\)$/, '')), type: t('validation.syntaxType'), node: null });
    return errors;
  }
  ...
  for (const prop of translationObjectNode.properties) {
    ...
    if (!KNOWN_PROPERTIES.has(keyName)) {
      errors.push({
        ...
        message: t('validation.unknownProperty', keyName),
        type: t('validation.structure'),
        node: prop,
      });
    }
  }
  ...
  for (const elementNode of allRuleNodes) {
    ...
    if (elementNode.type !== 'ArrayExpression' || elementNode.elements.length !== 2) {
      ...
      if (!isSpecificCheck) {
        errors.push({ file, line: elementNode.loc.start.line, lineContent: lines[elementNode.loc.start.line - 1].trim(), message: t('validation.invalidFormat'), type: t('validation.structure'), node: elementNode });
      }
      continue;
    }
 ...
 * @param {object} [options={}] - 包含要执行哪些检查的选项对象。
 * @param {boolean} [options.silent=false] - 是否静默模式（不打印默认日志）。
 * @param {function} [options.onProgress] - 进度回调函数 (current, total, file) => void。
...
export async function validateTranslationFiles(options = {}) {
  ...
  for (let i = 0; i < totalFiles; i++) {
    ...
    // 调用进度回调
    if (onProgress) {
      onProgress(i + 1, totalFiles, path.basename(file));
    }
    ...
    try {
      ...
      if (!silent) {
        printValidationResults(errorsInFile, file, { checkEmpty, checkDuplicates, checkMissingComma, checkIdentical, checkSourceDuplicates });
      }

      // 将当前文件的错误合并到总错误列表中。
      allErrors = allErrors.concat(errorsInFile);
    } catch (e) {
      if (!silent) console.error(t('validation.fileProcessingError', file), e);
    }
  }
  return allErrors;
}
 ...
 * @description 打印校验结果的辅助函数。
 * @param {ValidationError[]} errorsInFile - 单个文件的错误列表
 * @param {string} file - 文件路径
 * @param {object} options - 检查选项
 */
export function printValidationResults(errorsInFile, file, options) {
  const { checkEmpty, checkDuplicates, checkMissingComma, checkIdentical, checkSourceDuplicates } = options;
